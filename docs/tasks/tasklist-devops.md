...
- **`docker-compose.yml`** — полный stack (iter 0 ✅); iter 1 добавит режим `image:` **в этом же файле**
...
**Агент:** `IMAGE_TAG=main make stack-up` (registry profile) поднимает stack без local build — только корневой compose.
