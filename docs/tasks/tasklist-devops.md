...
**Текущее состояние:** production stack + **CD ✅** · **область DevOps MVP закрыта (18/18)**.

Сводный план области: [impl/devops/plan.md](impl/devops/plan.md) · итог: [impl/devops/summary.md](impl/devops/summary.md)

**Production VPS:** `8460897` · `201.51.4.34` · Ubuntu 24.04 · SSH: [server/README.md § Проверка SSH](../../devops/server/README.md#проверка-ssh)

**Прогресс:** **18 / 18** задач · **iter 0–4 ✅**
...
| 4 | Автоматизация деплоя (GHA → VPS) | 16–18 | ✅ Done | [plan](impl/devops/iteration-4-deploy-ci/plan.md) · [summary](impl/devops/iteration-4-deploy-ci/summary.md) |
...
│   └── ghcr-login.md         # docker login ghcr.io (PAT, -p / stdin)
...
| 16 | GitHub Secrets + deploy key (manual guide) | ✅ Done | [план](impl/devops/iteration-4-deploy-ci/tasks/task-16-github-secrets/plan.md) · [summary](impl/devops/iteration-4-deploy-ci/tasks/task-16-github-secrets/summary.md) |
| 17 | GHA workflow deploy → VPS | ✅ Done | [план](impl/devops/iteration-4-deploy-ci/tasks/task-17-deploy-workflow/plan.md) · [summary](impl/devops/iteration-4-deploy-ci/tasks/task-17-deploy-workflow/summary.md) |
| 18 | E2E auto-deploy + docs | ✅ Done | [план](impl/devops/iteration-4-deploy-ci/tasks/task-18-deploy-verify/plan.md) · [summary](impl/devops/iteration-4-deploy-ci/tasks/task-18-deploy-verify/summary.md) |
...
- VPS `8460897` · IPv4 `201.51.4.34` · Ubuntu 24.04 · preset 4801 (ru-3)
- SSH verify: [server/README.md § Проверка SSH](../../devops/server/README.md#проверка-ssh)
...
- [x] Verify SSH + docs: [inventory.example.md](../../devops/server/inventory.example.md), [server/README.md § Проверка SSH](../../devops/server/README.md#проверка-ssh)
...
**Обязательная проверка итерации 3 (после 15):**

| # | Критерий | Команда / URL |
|---|----------|---------------|
| 1 | Docker + Compose; bootstrap воспроизводим | `docker compose version`; [bootstrap.sh](../../devops/server/bootstrap.sh) + [deploy/README](../../devops/deploy/README.md) |
| 2 | Pull без ошибок | `make stack-pull-registry` (не голый `docker compose pull`) |
| 3 | Stack up | `make stack-health`; `http://IP:8000/health`; `http://IP:3000` |

Подробно: [deploy/README § Обязательная проверка](../../devops/deploy/README.md#обязательная-проверка-dod-iter-3).

**Агент:** три критерия green; log в summary task 15.  
**Пользователь:** [smoke-test.md](../smoke-test.md) на VPS URL; login `@ivan_p`.

---

## Итерация 4 — Автоматизация деплоя (GHA → VPS) ✅

**Ценность:** push в `main` → обновление образов (iter 1) → deploy на VPS без ручного SSH.

**Критерии итерации:** ✅
- Secrets `DEPLOY_*` в GitHub (см. [github-secrets.md](../../devops/deploy/github-secrets.md))
- `.github/workflows/deploy.yml` — после Docker Publish
- E2E run [28166334358](https://github.com/zatulik2606/diaai/actions/runs/28166334358) ✅
...
- [x] Документ `devops/deploy/github-secrets.md`
- [x] Secrets через `gh secret set` (или UI)
- [x] Deploy public key на сервере (bootstrap)
- [x] Verify SSH deploy user
...
- [x] `.github/workflows/deploy.yml` — `workflow_run` Docker Publish + dispatch
- [x] SSH: pull, up, wait web, stack-health, smoke curl
- [x] `devops/deploy/README.md` § CD
...
- [x] Push `main` → publish + deploy
- [x] Deploy run [28166334358](https://github.com/zatulik2606/diaai/actions/runs/28166334358) ✅
- [x] Обновить plan, tasklist, impl/devops/summary.md
- [x] Закрыть iter 4 summary
...
|--|--|
| Прогресс | 18 / 18 |
| Production | http://201.51.4.34:3000 |
...
| [impl/devops/plan.md](impl/devops/plan.md) | план области DevOps |
| [impl/devops/summary.md](impl/devops/summary.md) | итог iter 0–4 |
| [devops/server/inventory.example.md](../../devops/server/inventory.example.md) | IP, OS, server id |
| [devops/server/README.md](../../devops/server/README.md) | bootstrap, **SSH verify** |
| [devops/deploy/README.md](../../devops/deploy/README.md) | layout + CD pipeline |
| [devops/deploy/github-secrets.md](../../devops/deploy/github-secrets.md) | GitHub Secrets CD |
| [devops/ci/README.md](../../devops/ci/README.md) | CI, образы, tags |
| [devops/ci/ghcr-login.md](../../devops/ci/ghcr-login.md) | `docker login ghcr.io` (PAT, `-p`, stdin) |
