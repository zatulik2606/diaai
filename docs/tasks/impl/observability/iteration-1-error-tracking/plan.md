...
| `glitchtip-telegram-bridge` + Dozzle | ✅ [`devops/monitoring/compose.yml`](../../../../../devops/monitoring/compose.yml) |
| env `GLITCHTIP_*`, `TELEGRAM_ALARM_*` | ✅ `.env.example` |
| Безопасный smoke endpoint | 📋 **task 01** — `/debug/glitchtip-test` |
...
- [ ] **Агент:** task 01–04 `plan.md` → реализация → `summary.md`
- [ ] Backend: `GET /debug/glitchtip-test` + Bearer → issue в GlitchTip `diaai-backend`
- [ ] Web: `GET /api/debug/glitchtip-test` + Bearer → issue в GlitchTip `diaai-web`
