...
**Прогресс:** **0 / 5** итераций · [`backend/README.md`](../../backend/README.md) · [`docker-compose.yml`](../../docker-compose.yml)

> **Skills:** итерация 2 — `postgresql-table-design`; итерация 3 — сверка с [ADR-001](../adr/adr-001-database.md) и [ADR-002](../adr/adr-002-backend-stack.md).
...
|---|----------|--------|--------|-----------|
| 1 | Сценарии и требования к данным | 01 | 📋 Planned | [plan](impl/database/iteration-1-user-scenarios/plan.md) · [summary](impl/database/iteration-1-user-scenarios/summary.md) |
...
|--------|----------|--------|-----------|
| 01 | Сценарии пользователя и требования к данным | 📋 Planned | [план](impl/database/iteration-1-user-scenarios/tasks/task-01-user-scenarios/plan.md) · [summary](impl/database/iteration-1-user-scenarios/tasks/task-01-user-scenarios/summary.md) |
...
→ [iteration-1-user-scenarios/plan.md](impl/database/iteration-1-user-scenarios/plan.md)
...
- [ ] Описать **5–7 базовых сценариев диабетика**: дневник питания/инсулина, вопрос ассистенту, динамика за период, рекомендации, запись к доктору, просмотр истории консультаций
- [ ] Описать **3–4 сценария доктора**: список пациентов, обзор динамики пациента, консультация (статус, комментарий), справочные заметки без назначения доз
- [ ] Для каждого сценария — таблица «экран / действие → данные (read) → сохранение (write)»
- [ ] Сверить с [idea.md](../idea.md), [vision.md](../vision.md#роли-и-сценарии), [api-contract.md](../api/api-contract.md)
- [ ] Выделить **MVP data scope** vs **backlog**

#### Актуализация документации

| Файл | Что обновить |
|------|--------------|
| `docs/data/user-scenarios.md` | **создать** — сценарии диабетика и доктора |
| `docs/data/data-requirements.md` | **создать** — матрица read/write по ролям |
| `docs/data-model.md` | раздел «Требования из сценариев», матрица сценарий → сущность |
| `docs/vision.md` | ссылка на `docs/data/` |
...
- `docs/data/user-scenarios.md`, `docs/data/data-requirements.md`
...
- [ ] ER-диagram (Mermaid) в `docs/data/schema-er.md`; mapping «логика → таблица/колонка»
- [ ] Diff MVP → целевая схема: новые таблицы, расширение `users`
- [ ] Review по [postgresql-table-design](../../.agents/skills/postgresql-table-design/SKILL.md) → `docs/data/schema-review.md`
...
| Файл | Что обновить |
|------|--------------|
| `docs/data/schema-er.md` | **создать** — ER + физическая схема |
| `docs/data/schema-review.md` | **создать** — pass/warn/fix из skill review |
...
- `docs/data/schema-er.md`, `docs/data/schema-review.md`
- обновления: `docs/data-model.md`

### Definition of Done — итерация 2

**Self-check (агент):** ER покрывает итерацию 1; для каждой FK — индекс; время — `TIMESTAMPTZ`; review задокументирован; diff с `001_initial_schema.py` явный; согласовано с [ADR-001](../adr/adr-001-database.md).
...
**Self-check (агент):** `make db-reset && make test` green; репозитории покрыты тестами; assistant/events пишут в новые таблицы где предусмотрено; нет RAM persistence доменных данных.
...
|----------|----------|----------|
| 1 | Сценарии + матрица данных | `docs/data/user-scenarios.md`, `data-requirements.md` |
| 2 | ER + PG review | `docs/data/schema-er.md`, `schema-review.md` |
| 3 | ADR + guide | `adr-003`, `docs/tech/database-access.md` |
| 4 | One-command окружение | `make db-reset`, `make db-inspect`, `make db-shell` |
| 5 | Backend на полной схеме | `make test`, seeded E2E, нет RAM persistence |
