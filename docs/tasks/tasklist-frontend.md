...
| Экран 2 — лидерборд | Рейтинг прогресса **пациентов с диабетом** по метрикам и периодам (роль доктора) |
| Глобальный чат (FAB) | Диалог с **ассистентом** (сценарий D2) |
| Матрица «студенты × уроки» | **Пациенты × периоды/метрики** (`progress_snapshots`) |

**Текущее состояние:** spec UI ✅ (iter 0) · backend `/api/v1/web/*` + seed v3 ✅ (iter 1) · `web/` каркас + auth BFF + app shell ✅ (iter 2) · patient dashboard 📋 plan ready (iter 3).

**Прогресс:** **3 / 10** итераций ✅ · **3 / 10** задач ✅ · **следующая:** iter 3 — [план](impl/frontend/iteration-3-patient-dashboard/plan.md) · [task-03](impl/frontend/iteration-3-patient-dashboard/tasks/task-03-patient-dashboard/plan.md) · [summary iter 2](impl/frontend/iteration-2-scaffold/summary.md) · [plan области](impl/frontend/plan.md)
...
| Backend iter 4 (analytics API) | 📋 опционально | DTO dashboard/leaderboard закрыты `/api/v1/web/*` (iter 1 ✅) |
...
| 3 | Панель пациента с диабетом | 03 | 📋 Next | [plan](impl/frontend/iteration-3-patient-dashboard/plan.md) · [task-03 plan](impl/frontend/iteration-3-patient-dashboard/tasks/task-03-patient-dashboard/plan.md) |
...
| 03 | Панель пациента с диабетом (`/dashboard`) | 📋 Next | [план iter 3](impl/frontend/iteration-3-patient-dashboard/plan.md) · [план task 03](impl/frontend/iteration-3-patient-dashboard/tasks/task-03-patient-dashboard/plan.md) |
...
- [x] **Экран 2 — лидерборд:** переключатель таблица / scatter plot; таблица — место, progress bar, иконки метрик (ХЕ/БЖЕ/инсулин), топ-3 с медалями
...
- [x] Добавить доктора в БД: `@akozhin`, `telegram_id: 162684825`, `role: doctor`
...
**Self-check (агент):** миграция 003; seed idempotent; 8 endpoint'ов 200 + schema; doctor `@akozhin` в PG; `make test` green; skills review.
...
- [x] Plan-артефакты iter 3 + task 03 ([plan](impl/frontend/iteration-3-patient-dashboard/plan.md), [task-03 plan](impl/frontend/iteration-3-patient-dashboard/tasks/task-03-patient-dashboard/plan.md))
- [ ] 4 KPI-карточки с дельтой — данные **текущего пользователя** (ХЕ, вопросы, события питания, инсулин)
- [ ] Line chart — активность 14 дней (вопросы + food events по `telegram_id` из сессии)
- [ ] Таблица своих вопросов ассистенту — сортировка по времени
- [ ] Лента своих фиксаций (food / photo) — клик → детали
- [ ] Матрица прогресса — периоды × метрики (ХЕ, БЖЕ, инсулин) для пациента
- [ ] Backend: patient-scoped API или адаптация endpoint'ов *(gap: iter 1 — `/doctor/dashboard/*`)*
- [ ] Loading / error / empty states
- [ ] Smoke: `make backend-run` + `make db-reset` + login пациента из seed
...
- 📋 [План итерации](impl/frontend/iteration-3-patient-dashboard/plan.md) · [task-03 plan](impl/frontend/iteration-3-patient-dashboard/tasks/task-03-patient-dashboard/plan.md)
...
- [ ] Переключатель «Таблица / Карта» (tabs)
- [ ] Таблица: rank, progress bar, иконки метрик, медали топ-3
- [ ] Scatter plot по метрикам из API
- [ ] Responsive layout
...
**Self-check (агент):** оба режима на API; переключение без remount bugs.

**User-check (пользователь):** leaderboard открывается; топ-3 с медалями; scatter с hover/tooltip.

### Документы

- 📋 [План итерации](impl/frontend/iteration-4-leaderboard/plan.md) · [task-04 plan](impl/frontend/iteration-4-leaderboard/tasks/task-04-leaderboard/plan.md)
...
| 1 | Backend API + demo seed | ✅ 8 endpoints, `@akozhin`, seed v3 |
| 2 | Next.js scaffold | ✅ auth BFF, shell, FAB, `make web-*` |
| 3 | Панель пациента с диабетом | 📋 plan ready; patient API + `/dashboard` UI |
| 4 | Leaderboard | table + scatter |
