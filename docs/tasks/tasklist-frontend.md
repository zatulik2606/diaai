...
| Экран 2 — лидерборд | Рейтинг **пациентов с диабетом** (роль доктора): таблица / scatter plot; продукты, ХЕ, топ-5 БЖЕ медалями |
| Глобальный чат (FAB) | Диалог с **ассистентом** (сценарий D2) |
| Матрица «студенты × уроки» | **Пациенты × периоды/метрики** (`progress_snapshots`) |

**Текущее состояние:** spec UI ✅ (iter 0) · backend web API ✅ (iter 1) · web scaffold ✅ (iter 2) · patient dashboard ✅ (iter 3) · **leaderboard** — план + контракты ✅, реализация 📋 (iter 4).

**Прогресс:** **4 / 10** итераций ✅ · **4 / 10** задач ✅ · **активная:** iter 4 — [лидерборд](impl/frontend/iteration-4-leaderboard/plan.md) (планирование ✅) · [plan области](impl/frontend/plan.md) · [summary iter 4](impl/frontend/iteration-4-leaderboard/summary.md)
...
| Backend iter 4 (analytics API) | 📋 опционально | leaderboard DTO iter 4 — расширить `/api/v1/web/leaderboard` (products + bje_medal) |
...
| 4 | Лидерборд | 04 | 🚧 Next | [plan](impl/frontend/iteration-4-leaderboard/plan.md) · [summary](impl/frontend/iteration-4-leaderboard/summary.md) |
...
| 04 | Лидерборд | 🚧 Next | [план](impl/frontend/iteration-4-leaderboard/tasks/task-04-leaderboard/plan.md) · [summary iter](impl/frontend/iteration-4-leaderboard/summary.md) |
...
Страница `/leaderboard`: переключатель таблица / scatter plot; таблица рейтинга пациентов с продуктами, ХЕ и медалями топ-5 по БЖЕ.
...
- [x] Summary итерации (планирование)
...
- [ ] Backend: `products[]` + `bje_medal`, убрать legacy `metrics`/`medal`
- [ ] Переключатель «Таблица / Карта» (tabs)
- [ ] Таблица: место (rank), progress bar, иконки по потребляемым продуктам и количеству ХЕ
- [ ] Медальки топ-5 по БЖЕ на эти продукты
...
**User-check (пользователь):** leaderboard открывается; иконки продуктов и ХЕ; топ-5 по БЖЕ с медальками; scatter с hover/tooltip.

### Документы

- 🚧 [План итерации](impl/frontend/iteration-4-leaderboard/plan.md) · [summary](impl/frontend/iteration-4-leaderboard/summary.md) · [task-04 plan](impl/frontend/iteration-4-leaderboard/tasks/task-04-leaderboard/plan.md)
...
| 4 | Leaderboard | table/scatter, продукты+ХЕ, топ-5 БЖЕ медалями | 🚧 spec/plan ✅ · UI/backend 📋 |
