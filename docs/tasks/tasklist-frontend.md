...
→ [iteration-5-assistant-chat/plan.md](impl/frontend/iteration-5-assistant-chat/plan.md) · [summary](impl/frontend/iteration-5-assistant-chat/summary.md)

### Цель

FAB-виджет и страница `/chat`: история, отправка, ответ LLM (doctor + diabetic); общий `AssistantChatProvider`.

### Задача 05 ✅

#### Состав работ (реализация)

- [x] UI чата: message list, input, send
- [x] BFF `GET /api/assistant/history` → backend history
- [x] BFF `POST /api/assistant/messages` → backend send
- [x] Optimistic UI / error handling + retry загрузки истории
- [x] `AssistantChatPanel` + `AssistantChatProvider` (FAB ↔ `/chat`)
- [x] Route `/chat` — full-page chat + `loading.tsx`

#### Definition of Done

**Self-check (агент):** send → reply; history после reload из PG; `make web-build` green.

**User-check (пользователь):** `ivan_p` → FAB или `/chat` → seed history; send → reply; сообщение из FAB видно на `/chat`.

### Документы

- ✅ [План итерации](impl/frontend/iteration-5-assistant-chat/plan.md) · [summary](impl/frontend/iteration-5-assistant-chat/summary.md) · [task-05 plan](impl/frontend/iteration-5-assistant-chat/tasks/task-05-assistant-chat/plan.md) · [task-05 summary](impl/frontend/iteration-5-assistant-chat/tasks/task-05-assistant-chat/summary.md)
