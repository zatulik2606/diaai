...
- пользователей (пациент с диабетом, доктор);
- диалогов и запросов;
...
**Минусы:** связи пациент с диабетом ↔ события ↔ консультации выражены слабее, чем в SQL; риск дублирования и рассинхронизации при multi-service доступе; команда и модель данных ([data-model.md](../data-model.md)) ориентированы на реляционную схему; две парадигмы (SQL + document) без явной выгоды.
