...
| Вопрос | Варианты |
|--------|----------|
| PhotoAnalysis | Отдельная таблица + FK к Request vs только `dialog_requests.media` JSON |
| ProgressSnapshot | Persist в PG vs materialized view / on-the-fly aggregation |
| Doctor User | Запись в `users` с `role=doctor` vs отдельная таблица staff |
| Patient–doctor link | Через Consultation only vs явная связь many-to-many |
