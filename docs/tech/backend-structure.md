...
**Тесты:** `make test` — **84** (67 backend + 17 bot). Маркеры: `integration` (HTTP+DB), `unit` (repos, metadata).
...
| Testing | httpx `AsyncClient`, `dependency_overrides`; markers `integration`/`unit`; **84** tests (`make test`) |
