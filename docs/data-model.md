...
Базовый перечень сущностей — минимальный набор для сопровождения пациента с диабетом: пользователи (пациент с диабетом, доктор), диалоги, запросы, **анализ фото по составу (ХЕ, БЖЕ, БЖУ)**, события питания и инсулина, рекомендации, снимки прогресса, консультации. Физическая схема PostgreSQL — [schema-er.md](spec/schema-er.md) (database iter 2).
...
| роль | пациент с диабетом / доктор | enum | `TEXT NOT NULL CHECK (role IN ('diabetic','doctor'))` |
...
**Назначение:** сессия общения пациента с диабетом с системой (бот или web); контекст для последующих ответов и фиксаций.

| Поле | Описание | Домен | PG (целевая) |
|------|----------|-------|--------------|
| идентификатор | ID диалога | UUID | `UUID` PK |
| пользователь | ссылка на пациента с диабетом | UUID | `user_id UUID NOT NULL` FK → users |
...
| пользователь | пациент с диабетом | UUID | `user_id UUID NOT NULL` FK |
...
**Назначение:** фиксация состояния пациента с диабетом за период — база для динамики (D3).
...
|------|----------|-------|--------------|
| идентификатор | ID | UUID | `UUID` PK |
| пациент с диабетом | кто записался | UUID | `diabetic_id UUID NOT NULL` FK |
...
- **Пользователь (пациент с диабетом)** → много **Диалогов**, **Запросов**, **Анализов фото**, **Событий питания**, **Событий инсулина**, **Снимков прогресса**, **Консультаций**, **Рекомендаций**
...
erDiagram
    ...
    User ||--o{ Consultation : "пациент с диабетом"
...
|---------|------------|------------------------|
| `users` | пациент с диабетом / доктор | `display_name`, `email`; partial UNIQUE `telegram_id` |
